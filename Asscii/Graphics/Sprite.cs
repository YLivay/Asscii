...
namespace Asscii.Graphics
{
    ...
    {
        private DrawStep[] steps;
        private Array2D<CharInfo> canvas;

        private Sprite(DrawStep[] steps)
        {
            this.steps = steps;
            string[] lines = string.Join("", steps.Select(step => step.str).ToArray()).Split(new char[] { '\n' });
            Width = lines.Max(line => line.Length);
            Height = lines.Length;

            // Flattens the DrawSteps onto a canvas for faster rendering on FastConsole
            canvas = new Array2D<CharInfo>(Width, Height);
            DrawUsingSteps(canvas, new int?[Width * Height], 0, 0, 0);
        ...
        {
            string text = System.IO.File.ReadAllText(filepath, Encoding.Default).Replace("\r", "");
            ...
            string[] codes = text.Split(new string[] { "\x1B\x5B" }, StringSplitOptions.RemoveEmptyEntries);
            List<DrawStep> steps = new List<DrawStep>();
            ...
            char[] mSplit = new char[] { 'm' };
            char[] modifierSplit = new char[] { ';' };
            for (; i < codes.Length; i++)
            {
                string[] codesAndText = codes[i].Split(mSplit, 2);

                ConsoleColor fg = ConsoleColor.Gray, bg = ConsoleColor.Black;

                int[] modifiers = codesAndText[0]
                    .Split(modifierSplit)
                    .Select(modifier =>
                    {
                        int a;
                ...
                string trimmed = codesAndText[1].Trim(new char[] { '\n', '\0' });
                if (i > 0 && ((lastStep.fg == fg && lastStep.bg == bg) || trimmed.Length == 0))
                {
                    lastStep.str += codesAndText[1];
                }
                else
                {
        ...
        {
            DrawUsingBuffer(console.Buffer, console.DepthMap, x, y, depth);
        }
        ...
        public void DrawUsingSteps(Array2D<CharInfo> targetCanvas, int?[] depthMap, int x, int y, int depth)
        {
            ...
            foreach (DrawStep step in steps)
            {
                string[] lines = step.str.Split(new char[] { '\n' });
                for (int i = 0; i < lines.Length; i++)
                {
                    // If a line is too long, draw only part of the string.
                    string line = lines[i];
                    int startOffset = px < 0 ? -px : 0;
                    int endPx = px + line.Length;

                    if (endPx >= 0)
                    {
                        int endOffset = Math.Max(0, endPx - canvasWidth);
                        px = Math.Max(px, 0);
                        ...
                        foreach (char ch in line)
                        {
                            ...
                            int depthIdx = px + py * canvasWidth;
                            if (!depthMap[depthIdx].HasValue || depthMap[depthIdx] >= depth)
                            {
